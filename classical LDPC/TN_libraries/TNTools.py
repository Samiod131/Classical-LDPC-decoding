...
def mps_addition(mps1, mps2, optim_gauge=True, complex=False, renorm=False, norm_ord=2, max_len=False):
    '''
...
def ansatz_mps(mps_length, max_chi=20, phys_ind=2, complex=False):
    '''
    ...
    if complex:
        ...
        new_site = 1j*np.random.rand(1, phys_ind, min(phys_ind, max_chi)) + \
            np.random.rand(1, phys_ind, min(phys_ind, max_chi))
    else:
        new_site = np.random.rand(1, phys_ind, min(phys_ind, max_chi))
    ...
    for i in range(1, mps_length):
        ...
        if complex:
            ...
            new_site = 1j*np.random.rand(mps[-1].shape[2], phys_ind, bond_size) + \
                np.random.rand(mps[-1].shape[2], phys_ind, bond_size)
        else:
